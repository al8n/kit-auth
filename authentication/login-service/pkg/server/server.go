...
import (
	...
	loginpb "github.com/al8n/kit-auth/authentication/login-service/pb"
	loginendpoint "github.com/al8n/kit-auth/authentication/login-service/pkg/endpoint"
	loginservice "github.com/al8n/kit-auth/authentication/login-service/pkg/service"
	logintransport "github.com/al8n/kit-auth/authentication/login-service/pkg/transport"
...
type Server struct {
	...
	loginServer  loginpb.LoginServer
...
func (s *Server) Serve() (err error) {
	...
	var (
		...
		endpoints *loginendpoint.Set
		httpAddr  = ":" + cfg.HTTP.Port
		httpsAddr  = ":" + cfg.HTTPS.Port
		grpcAddr = ":" + cfg.RPC.Port
	)
	{
		service, err = loginservice.New(logger, ctrs, tracer)
		if err != nil {
			logger.Log("err", err)
			return err
		}

		endpoints, err = loginendpoint.New(service, logger, duration, tracer, cfg.Service.APIs)
		if err != nil {
			logger.Log("err", err)
			return err
		}
		...
		if cfg.HTTP.Runnable || cfg.HTTPS.Runnable {
			s.router = logintransport.NewHTTPHandler(*endpoints, tracer, logger, cfg.Service.APIs)
...
func (s *Server) serveRPC(address string,  endpoints loginendpoint.Set, logger log.Logger, tracer stdopentracing.Tracer, apis bootapi.APIs) (err error) {
	...
	s.loginServer = logintransport.NewGRPCServer(endpoints, tracer, logger, apis)

	loginpb.RegisterLoginServer(s.grpcServer, s.loginServer)
